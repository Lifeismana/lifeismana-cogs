...
def render_text_with_emoji(
    self, img, draw, coords: tuple() = (0, 0), text="", font: ImageFont = "", fill="black"
):
    ...
    emoji_set = "twemoji"
    if emoji_set == "apple":
        emojis = os.listdir(f"{bundled_data_path(self)}/emoji")
        for i in range(0, len(text)):
            ...
            if emoji == "u200D":
                ...
                emoji_img = (
                    Image.open(f"{bundled_data_path(self)}/emoji/{emoji}.png")
                    .convert("RGBA")
        ...
        emojis = os.listdir(f"{bundled_data_path(self)}/twemoji")
        for i in range(0, len(text)):
            char = text[i]
            if char == "\n":
                coords = (initial_coords[0], coords[1] + emoji_size)
            ...
            if emoji == "200d":
                ...
                emoji_img = (
                    Image.open(f"{bundled_data_path(self)}/twemoji/{emoji}.png")
                    .convert("RGBA")
                    .resize((emoji_size, emoji_size), Image.LANCZOS)
                )
                img.paste(emoji_img, (coords[0], coords[1] + 4), emoji_img)
